<?php
// Heading
$_['heading_title']       = 'Raport aktywności klientów';

// Text
$_['text_list']           = 'Lista aktywności klientów';
$_['text_address_add']    = '<a href="customer_id=%d">%s</a> dodał nowy adres.';
$_['text_address_edit']   = '<a href="customer_id=%d">%s</a> zaktualizował swój adres.';
$_['text_address_delete'] = '<a href="customer_id=%d">%s</a> usunął jeden ze swoich adresów.';
$_['text_edit']           = '<a href="customer_id=%d">%s</a> zaktualizował dane konta.';
$_['text_forgotten']      = '<a href="customer_id=%d">%s</a> poprosił o zmianę hasła.';
$_['text_login']          = '<a href="customer_id=%d">%s</a> zalogował się.';
$_['text_password']       = '<a href="customer_id=%d">%s</a> zaktualizował swoje hasło.';
$_['text_register']       = '<a href="customer_id=%d">%s</a> założył konto.';
$_['text_return_account'] = '<a href="customer_id=%d">%s</a> dodał zwrot biletu.';
$_['text_return_guest']   = '%s dodał zwrot biletu.';
$_['text_order_account']  = '<a href="customer_id=%d">%s</a> złożył <a href="order_id=%d">nowe zamówienie</a>.';
$_['text_order_guest']    = '%s złożył <a href="order_id=%d">nowe zamówienie</a>.';

// Column
$_['column_customer']     = 'Klient';
$_['column_comment']      = 'Uwagi';
$_['column_ip']           = 'Adres IP';
$_['column_date_added']   = 'Data dodania';

// Entry
$_['entry_customer']      = 'Klient';
$_['entry_ip']            = 'Adres IP';
$_['entry_date_start']    = 'Data rozpoczęcia';
$_['entry_date_end']      = 'Data zakończenia';